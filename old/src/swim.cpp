...
void FishTransform(fishRec * fish, float dt) {
...
void WhalePilot(fishRec * fish, float dt) {
	fish->phi = -20.0 * dt;
	fish->theta = 0.0;
	fish->psi -= 0.5 * dt;
	
	fish->x += WHALESPEED * fish->v * cos(fish->psi / RAD)
			* cos(fish->theta / RAD) * dt;
	fish->y += WHALESPEED * fish->v * sin(fish->psi / RAD)
			* cos(fish->theta / RAD) * dt;
	fish->z += WHALESPEED * fish->v * sin(fish->theta / RAD) * dt;
}

void SharkPilot(fishRec * fish, float dt) {
	static int sign = 1;
	...
	fish->y += SHARKSPEED * fish->v * sin(fish->psi / RAD)
			* cos(fish->theta / RAD) * dt;
	fish->z += SHARKSPEED * fish->v * sin(fish->theta / RAD) * dt;
}
